...
module.exports = {
  command: new SlashCommandBuilder()
    .setName('spawncoords')
    .setDescription('Find the spawn coordinates of the realm.')
  ...
  callback: async (interaction) => {
    ...
    const embed = new EmbedBuilder()
      .setTitle('Spawn Coords')
      .setDescription(`${second} ${loading} Joining realm\n${one}${none} Getting Spawn coordinates`)
      .setColor(65280)
      .setFooter({ text: `/spawncoords ` });
    ...
    try {
      ...
      client.on('start_game', async (packet) => {
        ...
        embed.setDescription(`${second}${check} Joining realm\n${second}${check} Getting Spawn coordinates \n${one}Coordinates: X: ${x.toFixed(2)}, Y: ${y.toFixed(2)}, Z: ${z.toFixed(2)}`);
        await message.edit({
          embeds: [embed]
        });
      });
